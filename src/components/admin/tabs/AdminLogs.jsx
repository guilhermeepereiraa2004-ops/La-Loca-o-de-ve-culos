...
const AdminLogs = ({ logs = [], isDbConnected = false }) => {
  ...
  const getActionBadgeColor = (action) => {
    ...
    if (act.includes('criar') || act.includes('adicionar')) return 'bg-emerald-500/10 text-emerald-700 border-emerald-100';
    if (act.includes('atualizar') || act.includes('editar')) return 'bg-amber-500/10 text-amber-700 border-amber-100';
    if (act.includes('apagar') || act.includes('deletar') || act.includes('excluir')) return 'bg-red-500/10 text-red-700 border-red-100';
    if (act.includes('login')) return 'bg-blue-500/10 text-blue-700 border-blue-100';
    if (act.includes('encerrar')) return 'bg-neutral-900 text-white border-neutral-800';
    return 'bg-black text-neutral-600 border-neutral-700';
  };
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h3 className="text-4xl font-black uppercase tracking-tighter text-white">Logs do Sistema</h3>
        ...
        <div className="flex items-center gap-3 shrink-0">
          {isDbConnected ? (
            <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 text-emerald-700 border border-emerald-100 rounded-full text-[10px] font-black uppercase tracking-widest">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-amber-500/10 text-amber-700 border border-amber-100 rounded-full text-[10px] font-black uppercase tracking-widest">
              <span className="relative flex h-2 w-2">
      ...
      {/* Filters Toolbar */}
      <div className="bg-[#0a0a0a] p-6 md:p-8 rounded-3xl border border-neutral-800 shadow-sm flex flex-col md:flex-row gap-6 items-center">
        {/* Search */}
        <div className="relative w-full md:flex-1">
          ...
          <input
            ...
            className="w-full pl-14 pr-6 py-4 bg-black rounded-2xl outline-none focus:ring-4 focus:ring-[#D4AF37]/10 focus:bg-[#0a0a0a] border-2 border-transparent focus:border-[#D4AF37]/30 transition-all text-sm font-bold placeholder:text-neutral-300"
          />
        </div>

        {/* Action filter */}
        <div className="flex items-center gap-3 w-full md:w-auto shrink-0 bg-black px-4 py-2 rounded-2xl border border-neutral-800">
          <Filter size={14} className="text-neutral-400" />
          <span className="text-[9px] font-black uppercase tracking-widest text-neutral-400">Ação</span>
          <select
            value={actionFilter}
            onChange={(e) => setActionFilter(e.target.value)}
            className="bg-transparent text-xs font-black uppercase text-neutral-200 outline-none cursor-pointer"
          >
      ...
      <div className="bg-[#0a0a0a] rounded-3xl border border-neutral-800 shadow-xl shadow-black/40 overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-[#D4AF37]/5 hover:border-neutral-700 group/card">
        {filteredLogs.length === 0 ? (
          <div className="text-center py-24 px-8">
            <div className="w-16 h-16 bg-black rounded-full flex items-center justify-center mx-auto mb-4 text-neutral-300">
              <Clock size={32} />
            </div>
          ...
          <>
            <div className="hidden lg:block overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[900px] md:min-w-0">
                <thead>
                  <tr className="bg-[#111111] text-[9px] uppercase tracking-[0.3em] text-neutral-400 font-black border-b border-neutral-800">
                ...
                <tbody className="divide-y divide-neutral-800 text-[11px] font-medium text-neutral-300">
                  {filteredLogs.map((log, idx) => (
                    <tr key={`${log.id || 'log'}-${idx}`} className="hover:bg-black/50 transition-colors">
                      {/* Timestamp */}
                      ...
                      <td className="px-4 py-3 xl:px-5 xl:py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-neutral-900 text-[#D4AF37] flex items-center justify-center font-black text-xs shrink-0 uppercase">
                            {String(log.userName || log.user_name || '?').charAt(0)}
                          </div>
                          <div>
                            <p className="font-black text-white leading-tight">{log.userName || log.user_name || 'Sistema'}</p>
                      ...
                      {/* Target Type */}
                      <td className="px-4 py-3 xl:px-5 xl:py-3.5 text-white font-black uppercase tracking-wider">
                        {log.targetType || 'Auth'}
                      </td>

                      {/* Description */}
                      <td className="px-4 py-3 xl:px-5 xl:py-3.5 text-neutral-300 font-sans leading-relaxed">
                        {log.description}
                        {log.targetId && (
                          <span className="ml-2 font-mono text-[9px] bg-neutral-900 text-neutral-400 px-1.5 py-0.5 rounded border border-neutral-700">
                      ...
                      <td className="px-4 py-3 xl:px-5 xl:py-3.5 text-right">
                        {log.details ? (
                          <button
                            ...
                            className="w-8 h-8 bg-black rounded-lg flex items-center justify-center text-neutral-400 hover:text-white hover:bg-[#D4AF37]/10 hover:border-[#D4AF37]/30 border border-transparent transition-all ml-auto"
            ...
            <div className="lg:hidden divide-y divide-neutral-800">
              {filteredLogs.map((log, idx) => (
                <div key={`${log.id || 'log'}-${idx}`} className="p-5 flex flex-col gap-4 hover:bg-black/50 transition-colors">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="font-black text-white leading-tight truncate">{log.userName || log.user_name || 'Sistema'}</p>
                  ...
                  <div className="space-y-1 text-xs">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <span className="font-black text-[#D4AF37] uppercase tracking-wider">{log.targetType || 'Auth'}</span>
                    </div>
                  ...
                  <div className="flex justify-between items-center gap-4 pt-1">
                    ...
                    {log.details ? (
                      <button
                        onClick={() => setSelectedLog(log)}
                        className="px-3 py-1.5 bg-black rounded-lg text-neutral-500 hover:text-white hover:bg-[#D4AF37]/10 border border-neutral-700 transition-all flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest active:scale-95"
                        title="Ver Detalhes do JSON"
                      >
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-[800] flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-300">
          ...
          <div className="bg-[#0a0a0a] w-full max-w-2xl rounded-3xl shadow-2xl relative z-10 overflow-hidden animate-in zoom-in-95 duration-300 max-h-[85vh] flex flex-col border border-neutral-800">
            
            {/* Modal Header */}
            <div className="p-8 border-b border-neutral-800 flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setSelectedLog(null)} className="w-10 h-10 bg-black flex items-center justify-center rounded-full text-neutral-400 hover:text-white hover:bg-neutral-100 transition-colors">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6">
              <div className="bg-black p-6 rounded-2xl border border-neutral-800 space-y-3">
            ...
            <div className="p-8 border-t border-neutral-50 bg-black/30 flex justify-end shrink-0">
              <button 
                ...
                className="px-10 py-4 bg-neutral-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-[#D4AF37] transition-all shadow-xl"
              >
                Fechar
              </button>
            </div>
