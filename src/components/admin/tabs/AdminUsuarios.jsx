  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h3 className="text-4xl font-black uppercase tracking-tighter text-white">Usuários do Sistema</h3>
          <p className="text-neutral-400 text-sm font-light mt-1">Gerencie logins e permissões de acesso para funcionários.</p>
        </div>
        <button
          onClick={handleOpenAdd}
          className="flex items-center gap-3 bg-neutral-900 text-white px-8 py-4 rounded-xl text-[10px] uppercase tracking-[0.2em] font-black hover:bg-[#D4AF37] transition-all shadow-xl"
        >
      ...
      <div className="bg-neutral-900 rounded-3xl p-8 border border-[#D4AF37]/20 flex items-center justify-between flex-wrap gap-6">
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-[#D4AF37] rounded-2xl flex items-center justify-center text-white shadow-lg">
            <Shield size={28} />
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="bg-[#D4AF37]/20 text-[#D4AF37] text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded">Administrador Master</span>
            </div>
      ...
      {systemUsers.length === 0 && (
        <div className="text-center py-20 bg-black rounded-3xl border border-neutral-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {systemUsers.map(user => (
          <div key={user.id} className="bg-[#0a0a0a] rounded-3xl border border-neutral-800 p-8 shadow-sm hover:shadow-xl transition-all group relative">
            <div className="flex justify-between items-start mb-6">
              <span className={`px-3 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${
                user.role === 'administrador' ? 'bg-amber-500/10 text-amber-600 border-amber-100' : 'bg-blue-500/10 text-blue-600 border-blue-100'
              }`}>
                {user.role === 'administrador' ? 'Administrador' : 'Funcionário'}
              </span>
              <div className="flex items-center gap-2">
                <button onClick={() => handleOpenEdit(user)} className="w-8 h-8 bg-black rounded-lg flex items-center justify-center text-neutral-400 hover:text-white hover:bg-neutral-100 transition-all">
                  <Edit size={14} />
                </button>
                <button onClick={() => {
                  setTargetDeleteId(user.id);
                  setShowPasswordModal(true);
                }} className="w-8 h-8 bg-black rounded-lg flex items-center justify-center text-neutral-300 hover:text-red-500 hover:bg-red-500/10 transition-all">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="w-12 h-12 bg-neutral-950 rounded-2xl flex items-center justify-center text-[#D4AF37] font-black text-lg shadow-lg">
                  ...
                  {ALL_MODULES.map(mod => {
                    ...
                    return (
                      <span key={mod.id} className={`text-[7px] font-black uppercase px-2 py-1 rounded-lg border ${
                        hasAccess ? 'bg-emerald-500/10 text-emerald-700 border-emerald-100' : 'bg-black text-neutral-300 border-neutral-800 line-through'
                      }`}>
                        {mod.icon} {mod.label}
                      </span>
                    );
      ...
      {showForm && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-500">
          ...
          <div className="bg-[#0a0a0a] w-full max-w-2xl rounded-3xl shadow-2xl relative z-10 overflow-hidden animate-in zoom-in-95 duration-500 max-h-[90vh] flex flex-col">
            {/* Modal Header */}
            <div className="p-8 border-b border-neutral-800 flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setShowForm(false)} className="text-neutral-300 hover:text-white transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8">
              <form onSubmit={handleSubmit} id="user-form" className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-black border-none p-4 rounded-xl outline-none focus:ring-2 focus:ring-[#D4AF37]/20 transition-all font-bold text-sm text-white"
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-black border-none p-4 pr-12 rounded-xl outline-none focus:ring-2 focus:ring-[#D4AF37]/20 transition-all font-bold text-sm text-white"
                      />
                ...
                {form.role === 'funcionario' && (
                  <div className="space-y-4 bg-black p-6 rounded-2xl border border-neutral-800">
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex gap-2">
                        <button type="button" onClick={() => setForm(prev => ({ ...prev, modules: ALL_MODULES.map(m => m.id) }))} className="text-[8px] font-black uppercase text-[#D4AF37] hover:underline">Todos</button>
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center gap-3 p-4 rounded-xl border text-left transition-all ${
                              isActive ? 'bg-[#0a0a0a] border-emerald-200 shadow-sm' : 'bg-neutral-100 border-transparent opacity-50'
                            }`}
                          >
                            <div className={`w-5 h-5 rounded flex items-center justify-center border shrink-0 transition-all ${isActive ? 'bg-emerald-500 border-emerald-500' : 'border-neutral-300 bg-[#0a0a0a]'}`}>
                              {isActive && <Check size={12} className="text-white" />}
                            </div>
                            <span className="text-xs font-black text-neutral-300">{mod.icon} {mod.label}</span>
                          </button>
                        );
                      })}
                    </div>
            ...
            {/* Modal Footer */}
            <div className="p-8 border-t border-neutral-50 bg-black/30 flex justify-end gap-4 shrink-0">
              <button type="button" onClick={() => setShowForm(false)} className="px-8 py-4 text-[10px] font-black uppercase tracking-widest text-neutral-400 hover:text-white transition-all">
                Cancelar
              </button>
              <button form="user-form" type="submit" className="px-12 py-4 bg-neutral-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-[#D4AF37] transition-all shadow-xl">
