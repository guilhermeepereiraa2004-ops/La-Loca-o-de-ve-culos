...
const InspectionDetailModal = ({ inspection, onClose, onCloseContract, rentals = [] }) => {
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 md:p-8 animate-in fade-in duration-500">
      ...
      {selectedMedia && (
        <div className="fixed inset-0 z-[700] flex items-center justify-center p-10">
          ...
          <div className="relative z-10 max-w-full max-h-full">
            ...
            <button onClick={() => setSelectedMedia(null)} className="absolute -top-12 right-0 text-white flex items-center gap-2 font-black uppercase text-[10px] tracking-widest hover:text-[#D4AF37] transition-colors">
      ...
      <div className="bg-[#0a0a0a] w-full max-w-6xl h-full max-h-[90vh] rounded-3xl shadow-2xl relative z-10 overflow-hidden flex flex-col animate-in zoom-in-95 duration-500">
        {/* Header */}
        <div className="p-8 md:p-10 border-b border-neutral-800 flex justify-between items-center bg-[#0a0a0a] shrink-0">
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 bg-neutral-900 rounded-2xl flex items-center justify-center text-[#D4AF37] shadow-2xl shadow-[#D4AF37]/20">
              <ClipboardCheck size={28} />
            </div>
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border ${
                  inspection.type === 'Entrega' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-100' :
                  inspection.type === 'Devolução' ? 'bg-blue-500/10 text-blue-600 border-blue-100' :
                  inspection.type === 'Coleta' ? 'bg-amber-500/10 text-amber-600 border-amber-100' : 'bg-black text-neutral-400 border-neutral-800'
                }`}>
                  Vistoria de {inspection.type}
                </span>
              </div>
              <h4 className="text-3xl font-black uppercase tracking-tighter text-white">Dossiê Técnico de Vistoria</h4>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-neutral-900 p-6 rounded-3xl border border-[#D4AF37]/20 flex flex-col justify-between">
              <p className="text-[8px] uppercase tracking-[0.2em] text-[#D4AF37] font-black mb-4">Veículo</p>
            ...
            <div className="bg-black p-6 rounded-3xl border border-neutral-800">
              <p className="text-[8px] uppercase tracking-[0.2em] text-neutral-400 font-black mb-4">Quilometragem</p>
              <div className="flex items-center gap-3">
                <Gauge size={20} className="text-white" />
                <h5 className="text-2xl font-black text-white">{inspection.km} <span className="text-xs">KM</span></h5>
              </div>
            </div>
            ...
            <div className="bg-black p-6 rounded-3xl border border-neutral-800">
              ...
              <div className="flex items-center gap-3">
                ...
                <h5 className="text-2xl font-black text-white uppercase tracking-tight">{inspection.fuelLevel}</h5>
              </div>
            </div>
            ...
            <div className="bg-black p-6 rounded-3xl border border-neutral-800">
              ...
              <div className="flex items-center gap-3">
                ...
                <h5 className="text-2xl font-black text-white uppercase tracking-tight">{inspection.tireCondition}</h5>
              </div>
            </div>
          </div>
          ...
          {(inspection.externalCleanliness || inspection.internalCleanliness || inspection.lastOilChangeKm || inspection.nextOilChangeKm) && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-black p-8 rounded-2xl border border-neutral-800 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {inspection.lastOilChangeDate ? (
                    <div className="bg-[#0a0a0a] p-4 rounded-2xl border border-neutral-800">
                      ...
                      <p className="text-xs font-black text-white">{new Date(inspection.lastOilChangeDate + 'T12:00:00').toLocaleDateString('pt-BR')}</p>
                    </div>
                  ) : null}
                  {inspection.lastOilChangeKm ? (
                    <div className="bg-[#0a0a0a] p-4 rounded-2xl border border-neutral-800">
                      <p className="text-[7px] uppercase font-black text-neutral-400 mb-1">Última Troca (KM)</p>
                      <p className="text-xs font-black text-white">{(parseInt(inspection.lastOilChangeKm) || 0).toLocaleString()} KM</p>
                    </div>
                  ) : null}
                  {inspection.nextOilChangeKm ? (
                    <div className="bg-[#0a0a0a] p-4 rounded-2xl border border-neutral-800">
                      <p className="text-[7px] uppercase font-black text-neutral-400 mb-1">Próxima Troca (KM)</p>
                      <p className="text-xs font-black text-white">{(parseInt(inspection.nextOilChangeKm) || 0).toLocaleString()} KM</p>
                    </div>
                  ) : null}
                </div>
              </div>
          ...
          <section>
            <div className="flex items-center gap-3 mb-8">
              ...
              <h5 className="text-sm font-black uppercase tracking-widest text-white">Evidências Fotográficas</h5>
            </div>
              ...
              {[
                ...
                return (
                  <div key={slot.id} className="space-y-3">
                    ...
                    <div className="aspect-square bg-neutral-100 rounded-3xl overflow-hidden relative group border border-neutral-700 shadow-inner">
                      {photo ? (
                        <>
                          ...
                          <div className="absolute inset-0 bg-neutral-950/60 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                            <button 
                              ...
                              className="w-10 h-10 bg-[#0a0a0a] rounded-xl flex items-center justify-center text-white shadow-xl hover:scale-110 transition-transform"
                            >
                              <Eye size={18} />
                            </button>
                            <a href={photo.preview} download className="w-10 h-10 bg-[#D4AF37] rounded-xl flex items-center justify-center text-neutral-950 shadow-xl hover:scale-110 transition-transform">
                              <Download size={18} />
                            </a>
                          </div>
                        </>
                    ...
                    <>
                      ...
                      <div className="absolute inset-0 bg-neutral-950/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-2">
                        <button 
                          ...
                          className="px-6 py-3 bg-[#D4AF37] rounded-xl flex items-center justify-center text-neutral-950 shadow-xl hover:scale-105 transition-all text-[9px] font-black uppercase tracking-widest"
                        >
                          Reproduzir
                        </button>
                      </div>
          ...
          {inspection.photos?.additional && inspection.photos.additional.length > 0 && (
            <section className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              <div className="flex items-center gap-3 mb-8">
                <Camera size={18} className="text-[#D4AF37]" />
                <h5 className="text-sm font-black uppercase tracking-widest text-white">Fotos Adicionais</h5>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          ...
          {inspection.hasDamages && (
            <section className="animate-in fade-in slide-in-from-bottom-4 duration-700">
               <div className="flex items-center gap-3 mb-8">
                  ...
                  <h5 className="text-sm font-black uppercase tracking-widest text-white">Avarias Identificadas</h5>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                  {inspection.damages?.filter(d => d.photo || d.description)?.map((dmg, idx) => (
                    <div key={idx} className="bg-[#0a0a0a] p-6 rounded-3xl border border-neutral-800 shadow-sm flex flex-col gap-6">
                       ...
                       <div className="space-y-2">
                          ...
                          <p className="text-xs text-neutral-300 font-bold leading-relaxed">{dmg.description || 'Sem descrição detalhada.'}</p>
                       </div>
                    </div>
                  ))}
               </div>            </div>
          ...
          {inspection.deductions && inspection.deductions.length > 0 && (
            <section className="bg-neutral-900 p-10 rounded-3xl border border-[#D4AF37]/20 relative overflow-hidden">
              ...
              <div className="space-y-4 relative z-10">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {inspection.deductions.map((item, idx) => (
                      <tr key={idx} className="group hover:bg-white/5 transition-colors">
                        <td className="py-4 text-[10px] font-black text-[#D4AF37] uppercase">{item.category}</td>
                        <td className="py-4 text-[10px] font-bold text-neutral-400 uppercase tracking-tight">{item.description || '-'}</td>
                        <td className="py-4 text-center">
                          {item.isProportional && <span className="bg-[#D4AF37]/20 text-[#D4AF37] px-2 py-1 rounded text-[7px] font-black uppercase">Sim</span>}
                        </td>
                ...
                <div className="pt-6 border-t border-white/10 flex justify-between items-end">
                  ...
                  <div className="text-right">
                    ...
                    <h6 className="text-3xl font-black text-[#D4AF37] tracking-tighter">
          ...
          <section className="bg-black p-10 rounded-3xl border border-neutral-800">
            <div className="flex items-center gap-3 mb-6">
              <FileText size={18} className="text-[#D4AF37]" />
              <h5 className="text-sm font-black uppercase tracking-widest text-white">Observações e Notas Técnicas</h5>
            </div>
            <div className="bg-[#0a0a0a] p-8 rounded-2xl border border-neutral-800 min-h-[150px]">
        ...
        {inspection.type === 'Devolução' && hasActiveRental && (
          <div className="p-8 border-t border-neutral-50 bg-black/30 flex justify-end shrink-0">
            <button 
              onClick={() => onCloseContract(inspection)}
              className="flex items-center gap-3 px-16 py-5 bg-neutral-900 text-[#D4AF37] text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl hover:bg-[#D4AF37] hover:text-white transition-all shadow-xl"
            >
